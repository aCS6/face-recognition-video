        ...
        if not success:
            ...
            for (top, right, bottom, left), name in zip(face_locations, face_names):
                ...
                if name != 'Unknown' and name not in attendance:
                    ...
                    time_of_in = times[attendance.index(name)]
                    time_now = datetime.now()

                    difference = (time_now - time_of_in).total_seconds()

                    # check if comes infront of camera between 30 seconds
                    if difference > 30:
                        # check if that name exist at the out time dictionary
                        if name not in outDict:
                            outDict[name] = datetime.now().strftime("%H:%M:%S")
                        else:
                            # if again in(come) then delete that name
                            del outDict[name]

                file = open('attendance.csv', 'w', newline ='')
                with file:
                    # identifying header  
                    header = ['Roll','Name', 'In','Out']
                    ...
                    for i in range(len(attendance)):
                        name = attendance[i]
                        ins = times[i].strftime("%H:%M:%S") # format to HH:MM:SS
                        try:
                            out = outDict[attendance[i]]
                        except:
                            out = ""
                        ...
                        # print(f"Name:{name}\tIn:{ins}\tOut:{out}")
